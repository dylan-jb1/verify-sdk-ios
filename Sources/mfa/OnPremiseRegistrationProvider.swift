...
public class OnPremiseRegistrationProvider: MFARegistrationDescriptor {
    ...
    internal func initiate(with accountName: String, pushToken: String? = nil, additionalData: [String: Any]? = nil) async throws {
        ...
        parameters["tenant_id"] = tenantID.uuidString
    ...
    public func nextEnrollment() async -> EnrollableSignature? {
        ...
        return EnrollableSignature(biometricAuthentication: biometricAuthentication, algorithm: algorithm, dataToSign: self.tenantID.uuidString)
    }
    ...
    public func enroll() async throws {
        ...
        let name = "\(self.tenantID.uuidString).\(self.currentFactor.type.rawValue)"
        ...
        let signedData = try sign(name: name, algorithm: self.currentFactor.algorithm, dataToSign: self.tenantID.uuidString)
