...
public actor OnPremiseAuthenticatorService: MFAServiceDescriptor {
    ...
    public init(with accessToken: String, refreshUri: URL, transactionUri: URL, clientId: String, authenticatorId: UUID, certificateTrust: URLSessionDelegate? = nil) {
    ...
    public func completeTransaction(action userAction: UserAction = .verify, signedData: String) async throws {
        ...
        let data: [String: Any] = ["signedChallenge": signedData]
...
extension OnPremiseAuthenticatorService {
    ...
    private func createPendingTransaction(with result: TransactionResult, transactionId: String? = nil) async throws -> PendingTransactionInfo {
        ...
        if let identifier = identifiers.first(where: { $0.values.contains(authenticatorId.uuidString) }) {
        ...
        guard let keyHandle = verificationInfo.keyHandles.first, let factorId = keyHandle.components(separatedBy: ".").first else {
        ...
        let result = PendingTransactionInfo(id: transactionInfo.transactionId,
                                            ...
                                            factorID: UUID(uuidString: factorId) ?? UUID(),
                                            factorType: verificationInfo.type,
                                            dataToSign: verificationInfo.serverChallenge,
